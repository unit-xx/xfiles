...
class TraderSpiDelegate(TraderSpi):
    ...
    def rsp_qry_instrument(self,pinstrument):
        ...
        '''
        logging.info(u'instruments info: %s' % pinstrument.InstrumentID)
        self.insts.append(pinstrument.InstrumentID)
...
class myMdSpi(MdSpi):
    def __init__(self, instruments, broker_id,
        ...
        self.torec = [
                'TradingDay', 'UpdateTime', 'UpdateMillisec', 'InstrumentID',
                ...
                'BidPrice1', 'BidVolume1', 'AskPrice1', 'AskVolume1', 'LastPrice', 'Volume', 'OpenInterest', 'Turnover'
                ]
        self.qfn = 'dummy.log'
        self.qf = None
        self.qw = None
    ...
    def OnFrontDisConnected(self, reason):
        print "md, onFrontDisConnected:", reason

    def OnHeartBeatWarning(self, time):
        print "onHeartBeatWarning", time

    def OnFrontConnected(self):
        print "md session OnFrontConnected"
        self.user_login(self.broker_id, self.investor_id, self.passwd)
    ...
    def OnRspUserLogin(self, userlogin, info, rid, is_last):
        print "OnRspUserLogin", is_last, info.ErrorMsg, info.ErrorID
        if is_last and not self.isErrorRspInfo(info):
            today = self.api.GetTradingDay()
            print "today is", today
            self.subscribe_market_data(self.instruments)

        self.qfn = 'quote.%s.csv' % today
        logging.info('Recording in file %s', self.qfn)
        self.qf = open(self.qfn, 'a+b')
        self.qw = csv.writer(self.qf)
        self.qbuf = []
        self.qw.writerow(self.torec)

    def logout(self):
        req = ustruct.UserLogout(UserID=self.investor_id, BrokerID=self.broker_id)
        self.requestid+=1
        r = self.api.ReqUserLogout(req, self.requestid)

    def OnRspUserLogout(self, pUserLogout, pRspInfo, nRequestID, bIsLast):
        print 'md logout', str(pRspInfo)
    ...
    def OnRtnDepthMarketData(self, d):
        ...
        self.qbuf.append( [getattr(d, x) for x in self.torec] )
        if len(self.qbuf) > 120:
            self.qw.writerows(self.qbuf)
            self.qbuf = []
...
def main():
    ...
    while 1:
        ...
        nowd = now.date()
        if (nowd != TODAY):
            # update non-trading day list
            ntdstr = [x.strip() for x in open(NTDFN).readlines()]
            ntd = [datetime.datetime.strptime(x, '%Y-%m-%d').date() for x in ntdstr]
            TODAY = nowd
            if (TODAY in ntd or TODAY.isoweekday() in (6,7)):
                SKIPDAY = True
                logging.info('skip %s', nowd)
            else:
                SKIPDAY = False
        ...
        if (nowt < BEGINTIME) or (nowt > ENDTIME):
            if REC is True:
                ...
                mdspi.closelog()
                user.Release()
                logging.info('Stop Recording for %s', now.date())
        else:
            if REC is False and SKIPDAY is False:
                ...
                logging.info('Try login CTP for %s', now.date())
                tdspi.evt.wait()
                tdspi.fetch_instrument(exchid=u'CFFEX')
                time.sleep(1)

                insts = tdspi.insts
                trader.Release()
                # got instruments list here.
                ...
                user.RegisterSpi(mdspi)
                user.RegisterFront("tcp://180.169.112.50:41213")
                user.Init()

                logging.info('Start Recording for %s', now.date())
