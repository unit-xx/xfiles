...
class session:
    ...
    def initheader(self):
        self.data = {}
        self.data["version"] = "KDGATEWAY1.2"
        self.data["user_code"] = ""
...
class response:
    ...
    def deserialize_payload(self):
        ...
        sections = [x.upper() for x in tmp[0:self.sectionnumber]]
        secrange = range(0, self.sectionnumber)
        records = []
        start = self.sectionnumber
        end = start + self.sectionnumber
        for i in range(self.recordnumber):
            r = {}
            r1 = zip(secrange,tmp[start:end])
            r2 = zip(sections,tmp[start:end])
            r.update(r1)
            r.update(r2)
            records.append(r)
...
class SubmitOrderReq(request):
    code = "403"
    paramlist = ["user_code", "market", "secu_acc", "account", "seat", "secu_code", "trd_id", "price", "qty", "biz_no", "ext_inst", "ext_rec_num", "op_remark", "match_seat", "match_num", "order_type"]
    pass
...
class QueryOrderResp(response):
    def getTotal(self):
        ...
        if len(self.records) == 0:
            ...
            for r in self.records:
                withdrawnqty = withdrawnqty + int(r["WITHDRAWN_QTY"])
                dealcount = dealcount + int(r["MATCHED_QTY"])
                dealamount = dealamount + float(r["MATCHED_AMT"])
            try:
