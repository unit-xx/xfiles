...
sprdfn = paste('sprdtrd1', qfn.base, 'pdf', sep='.')
...
for (i in 1:length(days))
{
  ...
  # 1. static mean using wsize
  cc = mean(dsprd$midsprd[1:wsize,])
  tsprd = dsprd$midsprd[(wsize+1):NROW(dsprd),] - cc
  ...
  abline(h=c(0, mincost/2+estep, -mincost/2-estep)+cc)
  abline(v=index(dsprd)[wsize])
}
dev.off()
