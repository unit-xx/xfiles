...
by sending ping messages. User can send query or control
messages using client utility. Authentication agent resolves
authentication challenge from peers to help them log into
...
peer can log into remote machines automatically to recover
...
SMON uses epidemic approach to manage itself. A SMON peer
will periodically select a random machine in membership list
and maintains the peer running on that machine.  The
epidemic approach is selected for several reasons.  First,
it has inherent scalability. Second, the epidemic algorithm
is simple and easy to implement. By its simplicity, the SMON
system is less prone to runtime errors so that it can run
reliably for long time. Third, it is robust and resilient to
failure.

In detail, a SMON peer runs the pseudo-code in
...
is no SMON peer deployed and running yet. A first peer have
...
SMON system stops maintaining itself (line 2-3 in
