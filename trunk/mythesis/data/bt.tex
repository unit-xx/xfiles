...
途径。\onlinecite{bns}通过模拟指出，使用临近邻居选择可以降低BitTorrent
的跨ISP流量，同时提高下载性能。然而~\onlinecite{bns}模拟使用的网络拓扑
过于简单，并不基于真实的网络数据。\onlinecite{taming, zeroday}的工作使
用了不同的技术，使临近邻居选择可以很容易的应用与部署到实际分发系统。另
一方面，\onlinecite{Bharambe2006}的工作显示了，固定上传节点数目的带宽
...
我们首先测试了邻居节点选择对分发性能的影响。在测试中，节点每次向
