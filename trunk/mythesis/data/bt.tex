...
\begin{itemize}
  ...
  达到最优。在实际应用中，为了防止网络分割的发生，可以在邻居列表中包括
  少量常数个随机节点。
...
内上传的数据块数目最大。若设节点的上传带宽是$u$，已有$m$个来自其它节点
...
体上看，提高了系统的聚集吞吐量。图~\ref{fig:bwalloc_throughput}比较了
使用和不适用上传带宽分配时，系统聚集吞吐量的变化。
...
\textbf{结论}：动态上传带宽分配策略通过运行时动态调整上传节点数据，提
...
此造成性能有不同程度的变化。图~\ref{fig:diversity_bias}是不同的数据块
选择策略对数据块分布的影响。我们统计了在整体分发进度为30\%时，数据块在
系统内的分布情况。横轴是数据块的编号，纵轴是相同数据块在系统内的副本数
量。可以看出，在随机数据块选择时，数据块在系统内分布的不够均匀。而使用
LRF选择策略的情况则不同，基本上所有的数据块的副本数量都分布在一个小的
范围之内。
...
\ref{fig:biasandbwalloc}回答了这个问题。可以看出合并后，并不会产生很强
的叠加优化结果。这是因为，采用邻近邻居后，邻居节点之间比较近，相互的传
输带宽也比较好，因而再使用带宽分配优化效果并不明显。
